...
meta :: Has (State (Subst Type)) sig m => Type -> m Meta
...
unify t1 t2 = type' t1 t2
  ...
  type' = curry $ \case
    (VNe (Metavar v1) Nil Nil, VNe (Metavar v2) Nil Nil) -> flexFlex v1 v2
    (VNe (Metavar v1) Nil Nil, t2)                       -> solve v1 t2
    (t1, VNe (Metavar v2) Nil Nil)                       -> solve v2 t1
    (VType, VType)                                       -> pure ()
    (VType, _)                                           -> nope
