...
compileClauses level ctx (_A :-> _T) heads = X.lamRA' level $ \ _v _k -> case _A of
  ...
  One      -> (match (_Unit.to (const [])) heads >>= compileClauses level ctx _T) X..||. X.covarA (Free 0)
  _A :* _B -> match (getUnion (Union (_Pair.to (\ (p, q) -> [p, q])) <> Union (_Var._Nothing.to (const [Var Nothing, Var Nothing])))) heads >>= \ heads' ->
    ...
    X.letA (X.µRA (X.varA (Free 3) X..||. X.prdL2A (X.covarA (Free 0)))) (
      compileClauses level ctx _T heads' X..||. X.covarA (Free 2)))
  _A :+ _B -> do
    ...
    X.varA (Free 1) X..||. X.sumLA
      ...
      (X.µLA (compileClauses level ctx (_A :-> _T) (heads' `at` 0) X..||. X.covarA (Free 0)))
      (X.µLA (compileClauses level ctx (_B :-> _T) (heads' `at` 1) X..||. X.covarA (Free 0)))
