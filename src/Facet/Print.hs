...
instance U.ForAll Print Print where
  ...
  t >=> f = bind $ \ var -> let var' = prettyVar var in braces (space <> ann var' t <> space) </> arrow <+> prec FnR (f var')
...
instance U.Decl Print Print Print where
  ...
  t >-> f = bind $ \ var -> let var' = prettyVar var in parens (ann var' t) </> arrow <+> prec FnR (f var')
