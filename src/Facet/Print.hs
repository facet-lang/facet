...
instance U.ForAll Print Print where
  ...
  t >=> f = bind $ \ var -> let var' = prettyVar var in braces (space <> var' <+> group (align (colon <+> t)) <> space) </> arrow <+> prec FnR (f var')
...
instance U.Decl Print Print Print where
  ...
  t >-> f = bind $ \ var -> let var' = prettyVar var in parens (var' <+> group (align (colon <+> t))) </> arrow <+> prec FnR (f var')
