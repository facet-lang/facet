...
surface = Algebra
  ...
  , ann' = tm
  , case' = \ s ps -> align . group $ pretty "case" <+> setPrec Expr s </> block (commaSep (map (\ (p, b) -> align (embed (prec Pattern p </> arrow) </> b)) ps))
  , pcon = \ n ps -> parens (hsep (annotate Con n:toList ps))
  , tuple = tupled
  , decl = ann
  , defn = \ (a :=: b) -> a </> b
  , data' = block . group . concatWith (surround (hardline <> comma <> space))
