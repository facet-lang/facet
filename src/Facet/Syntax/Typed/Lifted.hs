...
lam
  ...
  => (forall env' . Permutable env' => Extends env env' -> env' (Either (repr None a) (Expr.Inst eff (repr (Sum eff sig) a))) -> m (env' (repr sig b)))
  -> m (env (repr sig (repr (Sum eff sig) a -> repr sig b)))
lam f = fmap Expr.lam <$> liftBinder f

lam0
  :: (Applicative m, Permutable env, Expr.Expr repr)
  => (forall env' . Permutable env' => Extends env env' -> env' (repr None a) -> m (env' (repr sig b)))
